...
void update_data()
{
    // Use the Trapezoidal (Heun's) method to update positions and velocities
    ANNOTATE_SITE_BEGIN(for_update_data);
    {
        for (auto &particle : particles)
        {
            ...
            // Prediction step using Euler's method
            float pred_vel_x = particle.vel_x + particle.acc_x * step_size;
            float pred_vel_y = particle.vel_y + particle.acc_y * step_size;
            float pred_vel_z = particle.vel_z + particle.acc_z * step_size;

            float pred_pos_x = particle.pos_x + particle.vel_x * step_size;
            float pred_pos_y = particle.pos_y + particle.vel_y * step_size;
            float pred_pos_z = particle.pos_z + particle.vel_z * step_size;

            // Calculate predicted accelerations based on the predicted positions and velocities
            float pred_acc_x, pred_acc_y, pred_acc_z;
            //compute_acceleration(pred_pos_x, pred_pos_y, pred_pos_z, pred_vel_x, pred_vel_y, pred_vel_z, &pred_acc_x, &pred_acc_y, &pred_acc_z);

            // Correction step
            float avg_acc_x = (particle.acc_x + pred_acc_x) / 2.0;
            float avg_acc_y = (particle.acc_y + pred_acc_y) / 2.0;
            float avg_acc_z = (particle.acc_z + pred_acc_z) / 2.0;

            particle.vel_x += avg_acc_x * step_size;
            particle.vel_y += avg_acc_y * step_size;
            particle.vel_z += avg_acc_z * step_size;

            particle.pos_x += (particle.vel_x + pred_vel_x) / 2.0 * step_size;
            particle.pos_y += (particle.vel_y + pred_vel_y) / 2.0 * step_size;
            particle.pos_z += (particle.vel_z + pred_vel_z) / 2.0 * step_size;
        }
    }
}
    ...
    float get_energy()
    {
        ...
        ANNOTATE_SITE_BEGIN(get_energy);
        {
            for (const auto &particle : particles)
            {
                ANNOTATE_ITERATION_TASK(get_energy_0);
                ...
                {
                    ANNOTATE_ITERATION_TASK(get_energy_0_0);
                    if (&particle != &other)
                    {
