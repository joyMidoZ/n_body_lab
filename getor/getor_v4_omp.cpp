...
constexpr int STEP_PRINT_MOD{10};
...
#pragma omp declare simd
    void update_data()
    {
        // Use the Runge-Kutta method to update positions and velocities
...
#pragma omp simd
            for (auto &particle : particles)
            {
                ...
                // k1
                float k1vx = particle.acc_x * step_size;
                float k1vy = particle.acc_y * step_size;
                float k1vz = particle.acc_z * step_size;

                float k1px = particle.vel_x * step_size;
                float k1py = particle.vel_y * step_size;
                float k1pz = particle.vel_z * step_size;

                // k2
                float k2vx = (particle.acc_x + k1px / 2) * step_size;
                float k2vy = (particle.acc_y + k1py / 2) * step_size;
                float k2vz = (particle.acc_z + k1pz / 2) * step_size;

                float k2px = (particle.vel_x + k1vx / 2) * step_size;
                float k2py = (particle.vel_y + k1vy / 2) * step_size;
                float k2pz = (particle.vel_z + k1vz / 2) * step_size;

                // k3
                float k3vx = (particle.acc_x + k2px / 2) * step_size;
                float k3vy = (particle.acc_y + k2py / 2) * step_size;
                float k3vz = (particle.acc_z + k2pz / 2) * step_size;

                float k3px = (particle.vel_x + k2vx / 2) * step_size;
                float k3py = (particle.vel_y + k2vy / 2) * step_size;
                float k3pz = (particle.vel_z + k2vz / 2) * step_size;

                // k4
                float k4vx = (particle.acc_x + k3px) * step_size;
                float k4vy = (particle.acc_y + k3py) * step_size;
                float k4vz = (particle.acc_z + k3pz) * step_size;

                float k4px = (particle.vel_x + k3vx) * step_size;
                float k4py = (particle.vel_y + k3vy) * step_size;
                float k4pz = (particle.vel_z + k3vz) * step_size;

                // Update velocities
                particle.vel_x += (k1vx + 2 * k2vx + 2 * k3vx + k4vx) / 6;
                particle.vel_y += (k1vy + 2 * k2vy + 2 * k3vy + k4vy) / 6;
                particle.vel_z += (k1vz + 2 * k2vz + 2 * k3vz + k4vz) / 6;

                // Update positions
                particle.pos_x += (k1px + 2 * k2px + 2 * k3px + k4px) / 6;
                particle.pos_y += (k1py + 2 * k2py + 2 * k3py + k4py) / 6;
                particle.pos_z += (k1pz + 2 * k2pz + 2 * k3pz + k4pz) / 6;
            }
        }
    }
