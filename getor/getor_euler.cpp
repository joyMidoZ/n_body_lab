...
private:
    ...
    void update_data()
    {
        // Use the Modified Euler method to update positions and velocities
        ANNOTATE_SITE_BEGIN(for_update_data);
        {
            for (auto &particle : particles)
            {
                ...
                // Correction step
                float avg_acc_x = (particle.acc_x + (particle.acc_x + pred_vel_x) / 2) * step_size;
                float avg_acc_y = (particle.acc_y + (particle.acc_y + pred_vel_y) / 2) * step_size;
                float avg_acc_z = (particle.acc_z + (particle.acc_z + pred_vel_z) / 2) * step_size;

                particle.vel_x += avg_acc_x;
                particle.vel_y += avg_acc_y;
                particle.vel_z += avg_acc_z;

                particle.pos_x += (particle.vel_x + pred_vel_x) / 2;
                particle.pos_y += (particle.vel_y + pred_vel_y) / 2;
                particle.pos_z += (particle.vel_z + pred_vel_z) / 2;
            }
        }
    }
